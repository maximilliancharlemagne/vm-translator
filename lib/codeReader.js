...
class codeReader {
  constructor(){
    ...
    this.pushHandler = (segment, num) => {
      ...
      codeToReturn.push(`//push ${segment} ${num}`)
      switch (segment) {
        case 'const':
    ...
    this.popHandler = (segment, num) => {
      num = parseInt(num)
      //handling a pop instruction
      let codeToReturn = []
      let startCode = [`@0`,`M=M-1`,`A=M`,`D=M`,`@5`,`M=D`]
      let endCode = [`D=M`, `@${num}`, `D=D+A`, `@6`, `M=D`, `@5`, `D=M`, `@6`, `A=M`, `M=D`]
      codeToReturn.push(`//pop ${segment} ${num}`)
      switch (segment) {
        case 'const':
        ...
        case 'argument':
          ...
          //A=M
          //D=M
          //@5 (store variable in temp 0)
          //M=D
          //@2 (base address of arg)
          //D=M
          //@num
          //D=D+A
          //@6 (store address in temp 1)
          //M=D
          //@5 (get the variable)
          //D=M
          //@6 (get the address)
    ...
    this.addHandler = () => {
      ...
      let codeToReturn = []
      codeToReturn.push(this.popHandler('temp','11'))
    }
  }
}

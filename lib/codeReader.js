...
class codeReader {
  constructor(aFilename){
    this.tempArr = aFilename.split('.')
    this.filename = this.tempArr[0]
    this.readCode = (anArray) => {
      ...
      for(let index in anArray){
        ...
        let op = subArray[0]
        if(op.includes('//')){
          arrayToReturn.push(subArray.join(' '))
        }
        else{
        switch (op) {
          case 'push':
            ...
            break;
          case 'pop':
            arrayToReturn.push(this.popHandler(subArray[1], subArray[2]))
            break;
          case 'add':
          ...
          case 'neg':
            ...
            break
          case 'eq':
            arrayToReturn.push(this.eqHandler())
            break
          case 'gt':
            arrayToReturn.push(this.gtHandler())
            break
          case 'lt':
            arrayToReturn.push(this.ltHandler())
            break
          case 'and':
            arrayToReturn.push(this.andHandler())
            break
          case 'or':
            arrayToReturn.push(this.orHandler())
            break
          case 'not':
            arrayToReturn.push(this.notHandler())
            break
          default:
            break;
        }
      ...
      arrayToReturn.push(`(Halt)`,`@Halt`,`0;JMP`)
      return arrayToReturn.join('\n')
    }
    this.pushHandler = function (segment, num) {
      num = parseInt(num)
      ...
      switch (segment) {
        case 'static':
          codeToReturn = [`@${this.filename}.${num}`,`D=M`]
    ...
    this.popHandler = function (segment, num) {
      num = parseInt(num)
      ...
      switch (segment) {
        case 'static':
          codeToReturn = [`@0`, `M=M-1`, `A=M`, `D=M`, `@${this.filename}.${num}`, `M=D`]
          break
        case 'const':
    ...
    this.subHandler = () => {
      ...
      codeToReturn.push(`@12`, `D=M`, `@11`, `D=D-M`, `@10`, `M=D`) //add R11 and R12 and store them in R10
      codeToReturn.push(this.pushHandler('temp', '5')) //push what's in temp 5, AKA R10
      return codeToReturn.join('\n')
    }
    this.negHandler = () => {
      let codeToReturn = [`//neg`,`@0`,`A=M`,`M=-M`]
      return codeToReturn.join('\n')
    }
    this.eqHandler = () => {
      let codeToReturn = []
      codeToReturn.push(`//eq`)
      //pop temp 0
      codeToReturn.push(this.popHandler('temp','0'))
      //pop temp 1
      codeToReturn.push(this.popHandler('temp', '1'))
      codeToReturn.push(`//eq code`,`@5
D=M
@6
D=D-M
@False
...
@7
M=D`)
      codeToReturn.push(this.pushHandler('temp','2'))
      codeToReturn.push(`@End`,`0;JMP`,`(False)`,this.pushHandler('const','0'),`(End)`)
      return codeToReturn.join('\n')
    }
    this.gtHandler = () => {
      let codeToReturn = []
      codeToReturn.push(`//gt`)
      //pop temp 0
      codeToReturn.push(this.popHandler('temp', '0'))
      //pop temp 1
      codeToReturn.push(this.popHandler('temp', '1'))
...
M=D`)
    ...
    this.andHandler = () => {
      let codeToReturn = []
      codeToReturn.push(`//and`)
      //pop temp 0
      codeToReturn.push(this.popHandler('temp', '0'))
      //pop temp 1
      codeToReturn.push(this.popHandler('temp', '1'))
      codeToReturn.push(`//and code`)
      codeToReturn.push(`@5`,`D=M`,`@6`,`D=D&M`,`@7`,`M=D`)
      codeToReturn.push(this.pushHandler(`temp`,`2`))
      return codeToReturn.join(`\n`)
    }
    this.orHandler = () => {
      let codeToReturn = []
      codeToReturn.push(`//or`)
      //pop temp 0
      codeToReturn.push(this.popHandler('temp', '0'))
      //pop temp 1
      codeToReturn.push(this.popHandler('temp', '1'))
      codeToReturn.push(`//or code`)
      codeToReturn.push(`@5`, `D=M`, `@6`, `D=D|M`, `@7`, `M=D`)
      codeToReturn.push(this.pushHandler(`temp`, `2`))
      return codeToReturn.join(`\n`)
    }
    ...
    this.notHandler = () => {
      let codeToReturn = []
      codeToReturn.push(`//not`)
      codeToReturn.push(`@0`,`A=M`,`M=~M`)
      return codeToReturn.join(`\n`)
    }
  }
}
